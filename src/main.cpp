...
#if defined(NDEBUG)
#error "Cryptodev cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucrdv) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zCRDV is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zCRDV spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCRDV without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zCRDV spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zCRDV spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are not supported by Cryptodev"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCRDV spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcrdv");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zCRDV mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight < 10) {
        nSubsidy = 2500000 * COIN; // 25 mil premine split up into 10 blocks to avoid maxpayout limit
    ...
    } else if (nHeight < 180000 && nHeight >= 11) {
        nSubsidy =  1 * COIN;
    } else if (nHeight < 705600 && nHeight >= 180000) {
        nSubsidy =  10 * COIN;
    } else if (nHeight < 1231200 && nHeight >= 705600) {
        nSubsidy =  9 * COIN;
    } else if (nHeight < 1756800 && nHeight >= 1231200) {
        nSubsidy =  8.10 * COIN;
    } else if (nHeight < 2282400 && nHeight >= 1756800) {
        nSubsidy =  7.3 * COIN;
    } else if (nHeight < 2808000 && nHeight >= 2282400) {
        nSubsidy =  6.6 * COIN;
    } else if (nHeight < 3333600 && nHeight >= 2808000) {
        nSubsidy =  5.9 * COIN;
    } else if (nHeight < 3859200 && nHeight >= 3333600) {
        nSubsidy =  5.3 * COIN;
    } else if (nHeight < 4384800 && nHeight >= 3859200) {
        nSubsidy =  4.6 * COIN;
    } else if (nHeight < 4910400 && nHeight >= 4384800) {
        nSubsidy =  4.3 * COIN;
    } else if (nHeight < 5436000 && nHeight >= 4910400) {
        nSubsidy =  3.8 * COIN;
    } else if (nHeight < 5961600 && nHeight >= 5436000) {
        nSubsidy =  3.4 * COIN;
    } else if (nHeight < 6487200 && nHeight >= 5961600) {
        nSubsidy =  3.1 * COIN;
    } else if (nHeight < 7012800 && nHeight >= 6487200) {
        nSubsidy =  2.8 * COIN;
    } else if (nHeight < 7538400 && nHeight >= 7012800) {
        nSubsidy =  2.5 * COIN;
    } else if (nHeight < 8064000 && nHeight >= 7538400){
        nSubsidy =  2.3 * COIN;
    } else if (nHeight < 8589600 && nHeight >= 8064000) {
        nSubsidy =  2 * COIN;
    } else if (nHeight < 9115200 && nHeight >= 8589600) {
        nSubsidy =  1.85 * COIN;
    } else if (nHeight < 9640800 && nHeight >= 9115200) {
        nSubsidy =  1.67 * COIN;
    } else if (nHeight < 10166400 && nHeight >= 9640800) {
        nSubsidy =  1.5 * COIN;
    } else if (nHeight < 10692000 && nHeight >= 10166400) {
        nSubsidy =  1.35 * COIN;
    } else if (nHeight < 11217600 && nHeight >= 10692000) {
        nSubsidy =  1.22 * COIN;
    } else if (nHeight < 11743200 && nHeight >= 11217600) {
        nSubsidy =  1.09 * COIN;
    } else if (nHeight < 12268800 && nHeight >= 11743200) {
        nSubsidy =  0.98 * COIN;
    } else if (nHeight < 12794400 && nHeight >= 12268800) {
        nSubsidy =  0.89 * COIN;
    } else if (nHeight < 13320000 && nHeight >= 12794400) {
        nSubsidy =  0.80 * COIN;
    } else if (nHeight < 13845600 && nHeight >= 13320000) {
        nSubsidy =  0.72 * COIN;
    } else if (nHeight < 14371200 && nHeight >= 13845600) {
        nSubsidy =  0.65 * COIN;
    } else if (nHeight < 14896800 && nHeight >= 14371200) {
        nSubsidy =  0.58 * COIN;
    } else if (nHeight < 15422400 && nHeight >= 14896800) {
        nSubsidy =  0.52 * COIN;
    } else if (nHeight < 15948000 && nHeight >= 15422400) {
        nSubsidy =  0.47 * COIN;
    } else if (nHeight < 16473600 && nHeight >= 15948000) {
        nSubsidy =  0.42 * COIN;
    } else if (nHeight < 16999200 && nHeight >= 16473600) {
        nSubsidy =  0.38 * COIN;
    } else if (nHeight < 17524800 && nHeight >= 16999200) {
        nSubsidy =  0.34 * COIN;
    } else if (nHeight < 18050400 && nHeight >= 17524800) {
        nSubsidy =  0.31 * COIN;
    } else if (nHeight < 18576000 && nHeight >= 18050400) {
        nSubsidy =  0.28 * COIN;
    } else if (nHeight < 19101600 && nHeight >= 18576000) {
        nSubsidy =  0.25 * COIN;
    } else {
        nSubsidy = 0.15 * COIN; // 4.7 y
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCRDVStake)
{
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Cryptodev
...
{
    RenameThread("cryptodev-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCRDVMinted()
{
...
void RecalculateZCRDVSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCRDV supply
...
bool RecalculateCRDVSupply(int nHeightStart)
{
...
{
    // Cryptodev: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZCRDVSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are not supported by Cryptodev"));
        }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZCRDVMinted();
        RecalculateZCRDVSpent();
        RecalculateCRDVSupply(Params().Zerocoin_StartHeight());
    }

    //Track zCRDV money supply in the block index
    if (!UpdateZCRDVSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zCRDV supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCrdvSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CRDV to zCRDV
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCRDV spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCRDV serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zCRDV stake block is less than allowed start height", __func__);

    if (CZCrdvStake* zCRDV = dynamic_cast<CZCrdvStake*>(stake)) {
        CBlockIndex* pindexFrom = zCRDV->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zCRDV stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zCRDV stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCRDV->GetValue()));
        if (nChecksum200 != zCRDV->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCRDV->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCRDV() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zCRDV fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCRDV mints and %d zCRDV spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Cryptodev: We use certain sporks during IBD, so check to see if they are
